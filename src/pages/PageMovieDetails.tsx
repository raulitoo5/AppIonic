...
import DetallePelisSkeleton from '../components/DetallePeliSkeleton';
      ...
      <IonHeader className='pantallaPrincipal'>
        <IonToolbar>
          ...
          {/** El operador ? o optinal chaining intenta acceder a la variable y en caso de ser
             ...
             */}
          <IonTitle className='tituloCentro'>
            {!cargando && `Película: ${datos?.title}`}
            {cargando && "Cargando datos de película..."}
          </IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent fullscreen className='pantallaPrincipal'>
        {cargando && <DetallePelisSkeleton />}
        {!cargando &&
          <IonGrid>
            <IonRow>
